...
(http://opensource.org/licenses/MIT) DATE	     : 2018-09-24
*/
...
void CubeMap::loadCubeMap(const std::string &folderPath)
{
    ...
    for (unsigned int i = 0; i < numSidesInCube; ++i)
    {
        ...
        unsigned char *data =
            stbi_load(currentFile.c_str(), &width, &height, &nComponents, 0);
...
// It will be filled in later, either through an image that is transformed to the
// right format or generated in some shader.
void CubeMap::generateCubeMap(const int width, const int height,
                              CubeMapType cubeType)
{
    ...
    case HDR_MAP:
        for (unsigned int i = 0; i < numSidesInCube; ++i)
        {
            glTexImage2D(GL_TEXTURE_CUBE_MAP_POSITIVE_X + i, 0, GL_RGB32F, width,
    ...
    case PREFILTER_MAP:
        for (unsigned int i = 0; i < numSidesInCube; ++i)
        {
            glTexImage2D(GL_TEXTURE_CUBE_MAP_POSITIVE_X + i, 0, GL_RGB16F, width,
                         height, 0, GL_RGB, GL_FLOAT, NULL);
        }
        glTexParameteri(GL_TEXTURE_CUBE_MAP, GL_TEXTURE_WRAP_S, GL_CLAMP_TO_EDGE);
        glTexParameteri(GL_TEXTURE_CUBE_MAP, GL_TEXTURE_WRAP_T, GL_CLAMP_TO_EDGE);
...
void CubeMap::convolveCubeMap(const unsigned int environmentMap,
                              const Shader &convolveShader)
{
    convolveShader.use();
                               ...
                               const Shader &filterShader)
{
    ...
    {
        // Mip levels are decreasing powers of two of the original resolution of the
        // cubemap
...
// This function seems kind of useless, I should check to see if it is being inlined
// away by the compiler and think of better ways to do this
