...
LICENSE      : This code is licensed under the MIT license (MIT)
(http://opensource.org/licenses/MIT) DATE	     : 2018-09-13
*/
...
// Sets the internal pointers to the screen and the current scene and inits the
...
bool RenderManager::preProcess()
{
    ...
    // Cubemap convolution TODO:: This could probably be moved to a function of the
    // scene or environment maps themselves as a class / static function
    ...
    currentScene->specFilteredMap.preFilterCubeMap(environmentID, captureRBO,
                                                   preFilterSpecShader);
    ...
    glFramebufferTexture2D(GL_FRAMEBUFFER, GL_COLOR_ATTACHMENT0, GL_TEXTURE_2D, id,
                           0);
    ...
    for (unsigned int i = 0; i < currentScene->pointLightCount; ++i)
    {
        ...
        currentScene->drawPointLightShadow(pointShadowShader, i,
                                           pointLightShadowFBOs[i].depthBuffer);
    }
    ...
    // As we add more error checking this will change from a dummy variable to an
    // actual thing
    return true;
}

// TODO:: some of the buffer generation and binding should be abstracted into a
// function
bool RenderManager::initSSBOs()
{
    // Setting up tile size on both X and Y
    sizeX = (unsigned int)std::ceil(DisplayManager::SCREEN_WIDTH / (float)gridSizeX);
    ...
    // Buffer containing all the clusters
    {
        ...
        glBufferData(GL_SHADER_STORAGE_BUFFER,
                     numClusters * sizeof(struct VolumeTileAABB), NULL,
                     GL_STATIC_COPY);
    ...
    // Setting up screen2View ssbo
    {
        ...
        screen2View.inverseProjectionMat =
            glm::inverse(sceneCamera->projectionMatrix);
        ...
        // Basically reduced a log function into a simple multiplication an addition
        // by pre-calculating these
        screen2View.sliceScalingFactor = (float)gridSizeZ / std::log2f(zFar / zNear);
        screen2View.sliceBiasFactor =
            -((float)gridSizeZ * std::log2f(zNear) / std::log2f(zFar / zNear));

        // Generating and copying data to memory in GPU
        glBufferData(GL_SHADER_STORAGE_BUFFER, sizeof(struct ScreenToView),
                     &screen2View, GL_STATIC_COPY);
    ...
    // Setting up lights buffer that contains all the lights in the scene
    {
        ...
        glBufferData(GL_SHADER_STORAGE_BUFFER, maxLights * sizeof(struct GPULight),
                     NULL, GL_DYNAMIC_DRAW);

        GLint bufMask = GL_READ_WRITE;

        struct GPULight *lights =
            (struct GPULight *)glMapBuffer(GL_SHADER_STORAGE_BUFFER, bufMask);
        PointLight *light;
    ...
    // A list of indices to the lights that are active and intersect with a cluster
    {
        unsigned int totalNumLights =
            numClusters * maxLightsPerTile; // 50 lights per tile max
        glGenBuffers(1, &lightIndexListSSBO);
        glBindBuffer(GL_SHADER_STORAGE_BUFFER, lightIndexListSSBO);

        // We generate the buffer but don't populate it yet.
        glBufferData(GL_SHADER_STORAGE_BUFFER, totalNumLights * sizeof(unsigned int),
                     NULL, GL_STATIC_COPY);
        glBindBufferBase(GL_SHADER_STORAGE_BUFFER, 4, lightIndexListSSBO);
        glBindBuffer(GL_SHADER_STORAGE_BUFFER, 0);
    }

    // Every tile takes two unsigned ints one to represent the number of lights in
    // that grid Another to represent the offset to the light index list from where to
    // begin reading light indexes from This implementation is straight up from Olsson
    // paper
    {
        ...
        glBufferData(GL_SHADER_STORAGE_BUFFER,
                     numClusters * 2 * sizeof(unsigned int), NULL, GL_STATIC_COPY);
    ...
    // Setting up simplest ssbo in the world
    {
        ...
        // in that grid Another to represent the offset
...
bool RenderManager::loadShaders()
{
    ...
    stillValid &=
        fillCubeMapShader.setup("cubeMapShader.vert", "buildCubeMapShader.frag");
    stillValid &=
        convolveCubeMap.setup("cubeMapShader.vert", "convolveCubemapShader.frag");
    stillValid &=
        preFilterSpecShader.setup("cubeMapShader.vert", "preFilteringShader.frag");
    stillValid &=
        integrateBRDFShader.setup("screenShader.vert", "brdfIntegralShader.frag");
    ...
    // Rendering
    stillValid &=
        depthPrePassShader.setup("depthPassShader.vert", "depthPassShader.frag");
    stillValid &= PBRClusteredShader.setup("PBRClusteredShader.vert",
                                           "PBRClusteredShader.frag");
    ...
    stillValid &=
        pointShadowShader.setup("pointShadowShader.vert", "pointShadowShader.frag",
    ...
    // Post-processing
    stillValid &=
        highPassFilterShader.setup("splitHighShader.vert", "splitHighShader.frag");
...
bool RenderManager::initFBOs()
{
    ...
    // Directional light
    stillValid &=
        dirShadowFBO.setupFrameBuffer(shadowMapResolution, shadowMapResolution);

    // Point light
    for (unsigned int i = 0; i < numLights; ++i)
    {
        stillValid &= pointLightShadowFBOs[i].setupFrameBuffer(shadowMapResolution,
                                                               shadowMapResolution);
    }
...
void RenderManager::render(const unsigned int start)
{
    ...
    if (ImGui::CollapsingHeader("Controls"))
    {
        ...
        ImGui::InputFloat3("Camera Pos",
                           (float *)&sceneCamera->position); // Camera controls
    ...
    multiSampledFBO.clear(GL_COLOR_BUFFER_BIT | GL_DEPTH_BUFFER_BIT,
                          glm::vec3(0.0f));
    ...
    // 5 - Actual shading;
    // 5.1 - Forward render the scene in the multisampled FBO using the z buffer to
    // discard early
